...
/**
 * Aggiornamento di un evento da parte dell'amministratore.
 * I campi ammessi (stessi del flusso venditore) sono: titolo, descrizione,
 * luogo, immagine_url, data_inizio, data_fine, attivo. Il negozio
 * dell'evento NON è spostabile da qui (la creazione con scelta del negozio
 * passa dalla route POST /amministratore/eventi).
 */
export async function PATCH(
  ...
  const esito = validaCampiEvento(body, { parziale: true });
  if (esito.errore) {
