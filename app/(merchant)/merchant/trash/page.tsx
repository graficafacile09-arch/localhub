...
export default function TrashPage() {
  ...
  async function handleRestore(storeId: string) {
    ...
    try {
      const res = await fetch(`/api/merchant/stores/${storeId}/restore`, { method: "POST" });
      if (!res.ok) {
        const err = await res.json().catch(() => null);
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              I negozi eliminati finiscono qui. Puoi ripristinarli in qualsiasi momento.
            </p>
          </div>
          <button
            type="button"
      ...
      {!loading && !error && stores !== null && stores.length === 0 && (
        <div className="flex min-h-[220px] flex-col items-center justify-center rounded-2xl border border-white/70 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Quando elimini un negozio, lo troverai qui e potrai ripristinarlo.
          </p>
        </div>
      )}
