...
drop policy if exists "scan_log insert own" on public.scan_log;
create policy "scan_log insert own"
  on public.scan_log for insert
  with check (user_id = auth.uid()::text);

drop policy if exists "scan_log select own" on public.scan_log;
create policy "scan_log select own"
