-- ═══════════════════════════════════════════════════════════════════════
-- InCittà — GLS + SPEDIZIONE GRATUITA (V1) — versione per SQL Editor
--
-- IDENTICA alla migration 20260908_gls_spedizione_gratuita.sql, con l'unica
-- differenza che NON contiene BEGIN;/COMMIT; espliciti: il Supabase SQL
-- Editor esegue già lo script dentro la propria transazione, quindi va
-- incollato ed eseguito come UNICA query (Run).
--
-- Obiettivo: aggiungere GLS come corriere nazionale (servizio "standard")
-- e introdurre la "spedizione gratuita" configurabile per metodo, SENZA
-- rompere Poste Italiane (standard/express), BRT o Corriere locale.
--
-- Principi:
--   1. Migration ADDITIVA: nessuna tariffa Poste/BRT esistente modificata,
--      nessun dato esistente cancellato.
--   2. GLS usa lo STESSO modello parametrico "peso reale → fascia → tariffa"
--      degli altri corrieri (nessun peso volumetrico, nessun supplemento CAP/
--      zona, nessuna IVA aggiuntiva). Prezzi = preventivi medi nazionali.
--   3. "spedizione gratuita" è una proprietà del METODO
--      (negozio_metodi_spedizione.spedizione_gratuita), riutilizzabile da
--      qualunque corriere a tariffa (Poste/BRT/GLS): quando attiva il costo
--      spedizione applicato al cliente è €0,00 (il totale ordine è calcolato
--      con spedizione 0). Il costo è SEMPRE calcolato server-side dalle RPC.
-- ═══════════════════════════════════════════════════════════════════════

-- ── 1. Spedizione gratuita per metodo (default false = tariffa normale) ────
alter table public.negozio_metodi_spedizione
  add column if not exists spedizione_gratuita boolean not null default false;

-- ── 2. Allowlist carrier/servizio: aggiunge GLS ────────────────────────────
alter table public.negozio_metodi_spedizione
  drop constraint if exists negozio_metodi_spedizione_carrier_check;

alter table public.negozio_metodi_spedizione
  add constraint negozio_metodi_spedizione_carrier_check
  check (carrier in ('poste_italiane', 'brt', 'locale', 'gls'));

alter table public.negozio_metodi_spedizione
  drop constraint if exists negozio_metodi_spedizione_servizio_check;

alter table public.negozio_metodi_spedizione
  add constraint negozio_metodi_spedizione_servizio_check
  check (
    (carrier = 'poste_italiane' and servizio in ('standard', 'express'))
    or (carrier = 'brt' and servizio = 'online')
    or (carrier = 'gls' and servizio = 'standard')
    or (carrier = 'locale' and servizio = 'locale')
  );

-- ── 3. SEED — corriere e servizio GLS ──────────────────────────────────────
...
select c.id, 'standard', 'standard', 'GLS', '24/48 ore'
from public.shipping_carriers c
where c.codice = 'gls'
on conflict (carrier_id, codice) do nothing;

-- ── 4. SEED — tariffe GLS (versione attiva; Poste/BRT invariati) ──────────
-- Fasce: 0-2, 2-5, 5-10, 10-20 kg (preventivi medi nazionali, IVA inclusa).
...
cross join (values
  ('gls', 'standard',      0,   2000,  9.90),
  ('gls', 'standard',   2000,   5000, 11.90),
  ('gls', 'standard',   5000,  10000, 14.90),
  ('gls', 'standard',  10000,  20000, 19.90)
...
-- ═════════════════════════════════════════════════════════════════════
-- 5. crea_ordine — GLS + spedizione gratuita (sostituisce 20260904)
-- ═════════════════════════════════════════════════════════════════════
...
-- ═════════════════════════════════════════════════════════════════════
-- 6. crea_ordine_carrello — GLS + spedizione gratuita (sostituisce 20260904)
-- ═════════════════════════════════════════════════════════════════════
