-- ═══════════════════════════════════════════════════════════════════════
-- InCittà — RIMBORSI PAGAMENTI V1 (preparazione atomica + rilascio)
--
-- Il rimborso end-to-end NON chiama il provider dentro PostgreSQL: la
-- chiamata HTTP al gateway avviene nel servizio applicativo
-- (lib/pagamenti/rimborsi.ts) tra DUE RPC atomiche:
--
--   1. pagamenti_prepara_rimborso  — SECURITY DEFINER, FOR UPDATE:
--        - verifica ownership (merchant owner O admin);
--        - verifica payment_status ∈ (paid, partially_refunded);
--        - verifica payment_provider gateway + payment_id presente;
--        - calcola il residuo: payment_amount − payment_refunded_amount;
--        - valida 0 < importo ≤ residuo (niente over-refund);
--        - PRENOTA l'importo incrementando payment_refunded_amount
--          (protegge anche dal doppio rimborso: un retry identico viene
--          rifiutato perché il residuo è già diminuito);
--        - ritorna i dati pagamento necessari al provider (provider,
--          payment_id, importo, residuo nuovo).
--
--   2. pagamenti_rimborso_annulla  — rilascio della prenotazione se la
--        chiamata al provider fallisce (decremento idempotente).
--
-- Dopo il successo del provider, il servizio applicativo porta lo stato a
-- refunded/partially_refunded con la RPC ESISTENTE aggiorna_payment_status
-- (macchina a stati già in produzione, mai duplicata). Il webhook del
-- provider resta la fonte autorevole definitiva (evento idempotente in
-- pagamenti_eventi) e sovrascrive payment_refunded_amount con l'importo
-- autoritativo del provider: nessun doppio conteggio.
--
-- Refund Stripe Connect / application fee: quando la piattaforma rimborsa
-- un payment intent creato con application_fee, Stripe rimborsa
-- AUTOMATICAMENTE anche l'application fee (comportamento default di
-- refunds.create, nessun transfer/reversal manuale): qui NON si inventa
-- alcuna gestione della commissione.
--
-- Principi:
--   - SECURITY DEFINER + REVOKE da anon/authenticated + GRANT service_role
--     (pattern aggiorna_stato_ordine / aggiorna_payment_status);
--   - migration ESCLUSIVAMENTE additiva; nessun dato esistente toccato;
--   - l'importo arriva SEMPRE validato dal server (mai dal browser);
--   - nessuna chiamata HTTP al provider in PostgreSQL.
-- ═══════════════════════════════════════════════════════════════════════

begin;

-- ── 1. RPC: PREPARA rimborso (prenotazione atomica + validazioni) ───────────
...
-- ── 2. RPC: RILASCIA la prenotazione (provider fallito) ────────────────────
create or replace function public.pagamenti_rimborso_annulla(
  p_ordine_id uuid,
  p_importo numeric
)
returns jsonb
...
-- ── 3. Permessi: SOLO service_role (pattern esistente) ─────────────────────
revoke execute on function public.pagamenti_prepara_rimborso(uuid, numeric, uuid) from public, anon, authenticated;
grant execute on function public.pagamenti_prepara_rimborso(uuid, numeric, uuid) to service_role;

revoke execute on function public.pagamenti_rimborso_annulla(uuid, numeric) from public, anon, authenticated;
grant execute on function public.pagamenti_rimborso_annulla(uuid, numeric) to service_role;
