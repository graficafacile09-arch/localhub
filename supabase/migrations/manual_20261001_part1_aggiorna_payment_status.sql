...
AS 'BEGIN
  ...
  DECLARE v_ordine record;
  DECLARE v_attuale text;
  DECLARE v_consentita boolean := FALSE;
  BEGIN
    ...
    IF v_ordine.stato = ''cancellato'' AND p_nuovo_stato IN (''paid'', ''authorized'') THEN
      RETURN jsonb_build_object(
        ''ok'', FALSE,
        ''codice'', ''ORDINE_ANNULLATO'',
        ''messaggio'', ''Ordine annullato: la conferma di pagamento e stata ignorata.''
      );
    END IF;
    ...
    IF v_ordine.payment_expires_at IS NOT NULL
      ...
      RETURN jsonb_build_object(
        ...
        ''messaggio'', ''Il pagamento e arrivato dopo la scadenza della sessione.''
      );
    END IF;
