-- ═══════════════════════════════════════════════════════════════════════
-- InCittà — PACCO / SPEDIZIONE lato negozio (V1, stile eBay)
--
-- Il peso e le dimensioni appartengono al PACCO configurato dal venditore,
-- NON al singolo prodotto. In V1 esiste un UNICO pacco per ordine/negozio:
-- il peso del pacco (negozi.pacco_peso_grammi) determina la tariffa
-- Poste/BRT. Il prodotto può restare senza peso (prodotti.peso_grammi resta
-- disponibile per compatibilità/future evoluzioni ma NON è più requisito).
--
-- CORRIERE LOCALE: invariato (prodotti.costo_spedizione_locale per prodotto,
-- MAX nell'ordine). Nessun backfill, nessun valore inventato.
-- Le RPC crea_ordine / crea_ordine_carrello vengono ridefinite (CREATE OR
-- REPLACE) SOLO nella parte di calcolo del peso: la firma di
-- calcola_tariffa_spedizione resta invariata.
-- ═══════════════════════════════════════════════════════════════════════

begin;

-- ── 1. Configurazione PACCO del negozio (nullable, nessun default) ────────
alter table public.negozi
  add column if not exists pacco_peso_grammi integer;

alter table public.negozi
  add column if not exists pacco_lunghezza_cm integer;

alter table public.negozi
  add column if not exists pacco_larghezza_cm integer;

alter table public.negozi
  add column if not exists pacco_altezza_cm integer;

alter table public.negozi
  add column if not exists pacco_peso_max_grammi integer;
...
begin
  ...
  loop
    ...
    -- V1 pacco unico per ordine: il peso Poste/BRT NON deriva più dalla
    -- somma dei pesi prodotto. È calcolato nella sezione 7 dal pacco del
    -- negozio (v_negozio.pacco_peso_grammi).
