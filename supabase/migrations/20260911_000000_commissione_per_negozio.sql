-- ============================================================================
-- COMMISSIONE PIATTAFORMA PER NEGOZIO
-- ============================================================================
-- NULL = fallback alla commissione globale della piattaforma.
-- Valori consentiti: 0% - 10%.
-- Gli ordini esistenti mantengono il proprio snapshot.
-- ============================================================================
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.negozi'::regclass
      and conname = 'negozi_commissione_percentuale_check'
  ) then
    alter table public.negozi
      add constraint negozi_commissione_percentuale_check
      check (
        commissione_percentuale is null
        or (
          commissione_percentuale >= 0
          and commissione_percentuale <= 10
        )
      );
  end if;
end
