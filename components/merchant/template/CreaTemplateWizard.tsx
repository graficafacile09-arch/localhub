...
export default function CreaTemplateWizard({ storeId, storeName, onClose, onCreated }: Props) {
  const [nome, setNome] = useState(`${storeName} Base`);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-900">Salva come Template</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-2">Cosa includere</label>
        ...
        <div className="mt-6 flex gap-2">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-blue-700 disabled:opacity-50"
          >
            {saving ? (
