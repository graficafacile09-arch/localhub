...
type Form = {
  ...
  codice_fiscale: string; email: string; sede_legale: string;
};

const EMPTY: Form = { denominazione_legale:"", forma_giuridica:"", partita_iva:"", codice_fiscale:"", email:"", sede_legale:"" };
...
export default function VenditoreIdentitaLegale({ storeId }: { storeId: string }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      {([["denominazione_legale","Denominazione legale *","sm:col-span-2","Es. Rossi Mario"],["forma_giuridica","Forma giuridica","","Es. Ditta individuale / SRL"],["partita_iva","Partita IVA *","","11 cifre"],["codice_fiscale","Codice fiscale","",""],["email","Email","","email@esempio.it"],["sede_legale","Sede legale *","sm:col-span-2","Via, numero civico, CAP, Comune (Provincia)"]] as const).map(([key,label,span,placeholder])=><label key={key} className={span}><span className="mb-1.5 block text-sm font-semibold text-slate-700">{label}</span><input value={form[key]} onChange={e=>set(key,key==="partita_iva"?e.target.value.replace(/\D/g,""):e.target.value)} placeholder={placeholder} maxLength={key==="partita_iva"?11:key==="codice_fiscale"?16:undefined} type={key==="email"?"email":"text"} className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-blue-500"/></label>)}
    </div>
