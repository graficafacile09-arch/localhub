...
/**
 * Zona Pericolosa dell'Area Commerciante.
 * Il commerciante può eliminare (soft delete) il PROPRIO negozio; il
 * ripristino è una funzione di piattaforma riservata all'amministratore
 * (Cestino nel pannello Amministratore).
 */
export default function ZonaPericolosaModule({ storeId }: Props) {
  ...
  async function handleSoftDelete() {
    if (!confirm("Eliminare questo negozio? Finirà nel Cestino della piattaforma e il ripristino potrà essere effettuato solo dall'amministratore.")) return;
    setDeleting(true);
    try {
    ...
    <ModuleShell icon={<Skull className="h-4 w-4" />} title="Zona Pericolosa" subtitle="Operazioni distruttive" id="zona-pericolosa">
      <div className="rounded-xl border border-red-200 bg-red-50 p-4">
        ...
        <p className="mt-2 text-xs leading-5 text-red-600">
          Il negozio verrà spostato nel Cestino della piattaforma. Il ripristino è riservato
          all&apos;amministratore: dopo l&apos;eliminazione non potrai più ripristinarlo tu.
        </p>
        <button
          type="button"
